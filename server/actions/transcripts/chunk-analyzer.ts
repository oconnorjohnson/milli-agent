...
const decisionTrackerFunc = async ({ chunk, MeetingId }: {chunk: string, MeetingId: number }): Promise<string | null> => { 
    const completion = await opeani.chat.completions.create({ 
        messages: [
            {
                role: "system",
                content: "The following is an excerpt from a transcription of a meeting in which two people are brainstorming the development of a sci fi tv show. Analyze the discussion and identify decisions or other parts of the discussion that contain actionable points or ideas that can be used to move forward with the project. List the main decisions or points of discussion in a clear and concise manner.",
            },
            {
                role: "user", 
                content: chunk || ""
